	...
	turn() {
		...
		if (this.target === null || (!Util.hasResource(this.target))) {
			...
			for (var i = 0; i < robots.length; i++) {
				var r = robots[i];
				if (!this.controller.isRadioing(c)) {
					continue;
				}
				if (r.signal_radius === 5000) {
					this.target = Util.decodePosition(r.signal);
					this.controller.log("we're rushin: " + this.target);
				}
			}
		}
		...
		if (visibleEnemies.length === 0) {
			// Don't see enemy
			var dijkstras = new Dijkstras(this.controller.map, currentPosition, totalMoves, totalMoveCosts);
			var stop = dijkstras.resolve((vector) => (vector.getDistanceSquared(this.target) < 9 && (!Util.hasResource(vector)) && (!Util.isNextToCastleOrChurch(vector))));
			...
			if (bestEnemy != null) {
				...
				if (bestEnemyCanAttack && bestEnemy.unit !== SPECS.PROPHET &&
					...
					if (bestMove == null) {
						...
						var offset = bestEnemyPosition.subtract(currentPosition);
						return this.controller.attack(offset.x, offset.y);
					} else {
						// We can kite
				...
				if (bestMove == null) {
					...
					var dijkstras = new Dijkstras(this.controller.map, currentPosition, totalMoves, costs);
					var stop = dijkstras.resolve((vector) => (vector.getDistanceSquared(this.target) < 9 && (!Util.hasResource(vector)) && (!Util.isNextToCastleOrChurch(vector))));
					var move = Util.getMove(dijkstras, currentPosition, stop);
					if (!move.isZero()) {
						return this.controller.move(move.x, move.y);
